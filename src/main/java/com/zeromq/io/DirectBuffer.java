...
/**
 * Direct buffer which can wrap a byte[] or a {@link ByteBuffer} that is heap or
 * direct allocated for direct access with native types.
 */
public class DirectBuffer {
    ...
    /**
     * Attach a view to a {@link ByteBuffer} for providing direct access, the
     * {@link ByteBuffer} can be heap based or direct.
     * 
     * @param buffer
     *            to which the view is attached.
     */
    ...
    /**
     * Check that a given limit is not greater than the capacity of a buffer
     * from a given offset.
     * 
     * Can be overridden in a DirectBuffer subclass to enable an extensible
     * buffer or handle retry after a flush.
     * 
     * @param limit
    ...
    public void checkLimit(final int limit) {
        if (limit > capacity) {
            final String msg = String.format("limit=%d is beyond capacity=%d",
                    Integer.valueOf(limit), Integer.valueOf(capacity));
     ...
     * The duplicate {@link ByteBuffer} shares the underlying memory so all
     * changes are reflected. If no {@link ByteBuffer} is attached then one will
     * be created.
     * 
    ...
    public ByteBuffer duplicateByteBuffer() {
        ...
        if (null == byteBuffer) {
            if (null != byteArray) {
                ...
                duplicate = BitUtil.resetAddressAndCapacity(
                        ByteBuffer.allocateDirect(0), addressOffset, capacity);
            }
        } else {
    ...
    /**
     * Get from the underlying buffer into a supplied byte array. This method
     * will try to fill the supplied byte array.
     * 
     * @param index
    ...
    public int getBytes(final int index, final byte[] dst, final int offset,
        ...
        UNSAFE.copyMemory(byteArray, addressOffset + index, dst,
                BYTE_ARRAY_OFFSET + offset, count);
    ...
    public int putBytes(final int index, final byte[] src, final int offset,
            final int length) {
        final int count = Math.min(length, capacity - index);
        UNSAFE.copyMemory(src, BYTE_ARRAY_OFFSET + offset, byteArray,
    ...
    /**
     * Put an bytes into the underlying buffer for the view. Bytes will be
     * copied from current {@link java.nio.ByteBuffer#position()} to
     ...
     * @param length
     *            of the source buffer in bytes to copy
     * @return count of bytes copied.
     */
    public int putBytes(final int index, final ByteBuffer srcBuffer,
        ...
        UNSAFE.copyMemory(srcByteArray, srcBaseOffset + srcOffset, byteArray,
                addressOffset + index, count);
