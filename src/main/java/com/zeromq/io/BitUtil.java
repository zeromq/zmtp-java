...
public class BitUtil {
     ...
     * 
     * <b>Note:</b> It is assumed a cleaner is not responsible for reclaiming
     * the memory under this buffer and that the caller is responsible for
     * memory allocation and reclamation.
     * 
     * @param byteBuffer
     *            to set the address on.
     * @param address
     *            to set for the underlying buffer.
     * @param capacity
     *            of the new underlying buffer.
    ...
    public static ByteBuffer resetAddressAndCapacity(
        ...
        if (!byteBuffer.isDirect()) {
            throw new IllegalArgumentException(
                    "Can only change address of direct buffers");
        }
        ...
        try {
            ...
            final Field capacityField = Buffer.class
                    .getDeclaredField("capacity");
            capacityField.setAccessible(true);
            capacityField.set(byteBuffer, Integer.valueOf(capacity));

            final Field cleanerField = byteBuffer.getClass().getDeclaredField(
                    "cleaner");
